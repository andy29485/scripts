...
PAT="=([0-9]+):"
if [[ $WIDTH =~ $PAT ]] ; then
  scale_tmp=${BASH_REMATCH[1]}
  if [[ $CROP =~ $PAT ]] ; then
    crop_tmp=${BASH_REMATCH[1]}
    if [[ scale_tmp -gt crop_tmp ]] ; then
      WIDTH="scale=$crop_tmp:-1:flags=lanczos"
    fi
  fi
fi
...
if [[ ! -z "$GIF" ]] ; then
  ...
  if [[ ! -z "$DEBUG" ]] ; then
    echo $convert -delay $DELAY/1000 -loop 0 "\"$tdir\"/ffout*.png" \"$tdir/out.gif\"
  fi
  $convert -delay $DELAY/0 -loop 0 "$tdir"/ffout*.png "$tdir/out.gif" > /dev/null 2>&1
  if [[ ! -z "$DEBUG" ]] ; then
  ...
  if [[ ! -z "$DEBUG" ]] ; then
    echo apngasm "\"$tdir\"/ffout*.png" -o \"$tdir/out.png\" -d $DELAY -l 0
  fi
  apngasm "$tdir"/ffout*.png -o "$tdir/out.png" -d $DELAY -l 0 > /dev/null 2>&1
  if [[ ! -z "$DEBUG" ]] ; then
